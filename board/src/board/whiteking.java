...
public class whiteking extends figure{
    ...
    whiteking(String name,int positionrow,int positioncol,boolean isWhite,boolean King){
    ...
    ArrayList<String> moveable(figure[][] board){
        ...
        String EnamyBlackKingPosition = trash.findBlackKing(board);
        int kingpositionrow = Integer.parseInt(String.valueOf(EnamyBlackKingPosition.charAt(1)));
        int kingpositioncol = trash1.turnToInt(EnamyBlackKingPosition.charAt(0));
        kingpositionrow -=1;
        ...
        ArrayList<String> enamymovelist = new ArrayList<>();
            for(int i=0;i<board.length;i++){
                for(int j=0;j<board.length;j++){
                    if(board[i][j] != null && board[i][j] != this && board[i][j].isIsWhite() == false && board[i][j].isKing() == false){   
